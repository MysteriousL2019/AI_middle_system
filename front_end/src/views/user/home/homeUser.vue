...
<template>
  <d2-container better-scroll type="card">
     <template slot="header">Model Management</template>
    <!-- Hello World<br>
    dataModel -->
      <el-card>
        <el-upload 
        ...
        <div slot="tip" class="el-upload__tip">Only allowed files ended with .pkl .npy .pt .pb .h5</div>
    ...
    methods: {
      async downloadFile(filename){
          ...
          if(confirmResult!='confirm'){
            ...
            // this.$http.delete
          console.log("点击了继续");
          const {data:res}= await this.$http.get("file/downLocal?remoteFileName="+filename+"&localFileName="+filename);
      ...
      async deleteFile(fileName){
          ...
          const {data:res} = await this.$http.delete("file/delete?path="+fileName);
          if(res!="success"){
      ...
      async getList(){
          const {data:res}=await this.$http.get("file/listObject");
      ...
      beforeUpload(file) {
        ...
        // const isPkl = file.name.endsWith(".pkl") || file.name.endsWith(".npy") || file.name.endsWith(".pt") || file.name.endsWith(".pb") || file.name.endsWith(".h5");

        // if (!isPkl) {
        //   this.$message.error('Only allowed files ended with .pkl .npy .pt .pb .h5')
        // }
        // return isPkl
      },
        //保存按钮
      async onSubmit (file) {
        ...
        //下面数据是我自己设置的数据,可自行添加数据到formData(使用键值对方式存储)
            ...
            this.logForm.operationType='updateModel';
            try {
                const {data:res} = await this.$http.post('http://localhost:9090/file/upload', formData, {
                  headers: {
